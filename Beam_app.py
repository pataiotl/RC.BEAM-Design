...
class RebarGroup:
    area: float
    centroid: float
    extreme_fiber: float        # distance from the SAME face as the group (top/bottom)
    width_req: float
    layers: list


# ---------------------------------------------------------------------------
# Session-state defaults  (defined ONCE)
# ---------------------------------------------------------------------------

DEFAULT_APP_STATE: dict = {
...
# ---------------------------------------------------------------------------
# Workspace save / load  (single canonical implementation)
# FIX #2: one definition only.
# FIX #3: "__json__:" prefix applied consistently so lists/dicts round-trip.
# ---------------------------------------------------------------------------

def _encode_cell(value):
    """Encode a Python value to an Excel-safe string, prefixing complex types."""
...
def _decode_cell(value):
    """Reverse of _encode_cell."""
    if isinstance(value, str) and value.startswith("__json__:"):
        return json.loads(value[len("__json__:"):])
    return value


def build_workspace_excel_bytes() -> bytes:
    """Serialize the full app state (and optional SAP data) to an .xlsx file."""
    output = BytesIO()
...
def load_workspace_excel(uploaded_excel) -> None:
    """Load app state from a workspace .xlsx file (supports both wide and legacy key/value layouts)."""
...
# FIX #1: sqlite3 is now imported at the top.  DB helpers kept but placed here once.
def build_workspace_db_bytes() -> bytes:
    """Serialize the full app state to a SQLite .db file."""
...
def load_workspace_db(uploaded_db) -> None:
    """Load app state from a SQLite workspace file."""
    with tempfile.NamedTemporaryFile(delete=False, suffix=".db") as tmp_db:
...
def get_rebar_group(
    n1, dia1, n2, dia2, n3, dia3,
    cover_clear, tie_dia, clear_space_input=25
) -> RebarGroup:
    """
    Build a RebarGroup measured from the NEAR face of the section
    (top face for top bars, bottom face for bottom bars).

    extreme_fiber = distance from near face to the outermost (first) layer centroid.
    centroid      = distance from near face to the area-weighted centroid.
    """
    if (n1 + n2 + n3) == 0:
    ...
    # extreme_fiber = outermost layer centroid (smallest y = closest to near face)
...
def calculate_beam_flexure(b, h, d, dt, d_prime, fc, fy, As, As_prime) -> dict:
    """
    ACI 318-19 flexural strength with iterative neutral-axis solver.

    FIX #4: Replaced the crude 0.1 mm decrement loop with a proper bisection
    solver that converges to within 0.01 mm.  The equilibrium function is
    monotone in c, so bisection is guaranteed to converge.

    dt = distance from compression face to the EXTREME tension steel layer
         (used only for the phi-factor strain check; must already be computed
          correctly by the caller – see FIX #7 in the design loop).
    """
    if As <= 0:
        return {
            "phi_Mn": 0, "passes_As_min": False, "is_ductile": False,
            "converged": True, "As_min": 0, "As_max_tc": 0,
            "passes_As_max_tc": False, "strain_class": "N/A",
    ...
    def _equilibrium(c):
        """Positive = compression exceeds tension (c is too large)."""
        a = min(beta1 * c, h)
        Cc = 0.85 * fc * a * b
        ...
        if d_prime <= a and eps_s_prime > 0:
            Cs -= As_prime * 0.85 * fc          # subtract concrete displaced by A's
        eps_s = ecu * (d - c) / c
        fs = min(fy, max(-fy, eps_s * Es))
        T = As * fs
        return (Cc + Cs) - T

    # Bisection: c_lo where equilibrium < 0, c_hi where equilibrium > 0
    c_lo, c_hi = 1.0, d
    converged = False

    if _equilibrium(c_lo) > 0:
        # Section is heavily over-reinforced; best estimate is c ≈ c_lo
        c = c_lo
        converged = False
    elif _equilibrium(c_hi) < 0:
        # Tension steel never yields even at d; extremely unlikely
        c = c_hi
        converged = False
    else:
        for _ in range(60):
...
def calculate_shear_torsion(
    b, h, d, fc, fyt, fyl, cover_clear,
    Vu_kN, Tu_kNm, n_legs, bar_dia, lambda_c
) -> dict:
    """
    ACI 318-19 shear + torsion design.

    FIX #5: phi_Vn is set to 0 when section_fails so the returned value
            correctly reflects failure (was previously non-zero even on fail).

    FIX #6: fyl is now clearly documented as the longitudinal steel fy used
            in Al_min.  Callers pass fy (main steel) which is correct per
            ACI 318-19 Eq. 9.6.4.2 — the code was correct but misleadingly named.
            Renamed to fyl_long internally to avoid confusion.

    FIX #8: s_min_steel now uses n_legs*A_leg / min_combined_ratio (correct
            ACI minimum Av/s applied to the full stirrup).
    """
    if d <= 0:
        return {
            "final_s": 0, "section_fails": True, "needs_torsion": False,
            "Al_req": 0, "Al_min": 0,
        }

    fyl_long = fyl          # longitudinal bar fy for Al_min  (ACI Eq. 9.6.4.2)
    ...
    # Vc per ACI 318-19 Table 22.5.5.1, row (a) — assumes Av ≥ Av,min is designed
    Vc = 0.17 * lambda_c * math.sqrt(fc) * b * d
    phi_Vc = phi_v * Vc

    # lambda_s is the size-effect factor from Table 22.5.5.1 row (c).
    # It applies only when Av < Av,min; kept here for display/reference.
    ...
    if needs_torsion:
        ...
        Al_min = max(
            0,
            (0.42 * math.sqrt(fc) * Acp / fyl_long)
            - (At_s_for_min * ph * (fyt / fyl_long)),
        )
        Al_req = max(At_s_req * ph * (fyt / fyl_long), Al_min)
    else:
        At_s_req = Al_req = Al_min = 0

    req_per_outer_leg = (Av_s_req / max(n_legs, 1)) + At_s_req
    s_calc = A_leg / req_per_outer_leg if req_per_outer_leg > 0 else 9_999

    # FIX #8: Minimum Av/s per ACI 318-19 Table 9.6.3.3 applied to the full
    # section width b.  Av = n_legs * A_leg, so:
    ...
    s_max_shear = (
        min(d / 4, 300) if Vs_req > (0.33 * math.sqrt(fc) * b * d)
        else min(d / 2, 600)
    )
    ...
    # FIX #5: zero out spacing AND capacity when the section fails web-crushing
    if section_fails or final_s < 50:
        final_s = 0
        phi_Vn = 0.0
    else:
        phi_Vn = phi_Vn_raw
    ...
    return {
        ...
        "phi_Vc": round(phi_Vc / 1_000, 1),
        "phi_Vn": round(phi_Vn, 1),
...
def calculate_development_length(db, fy, fc, is_top_bar, cover_clear, clear_spacing, lambda_c) -> dict:
    """ACI 318-19 development / lap / hook lengths (metric)."""
    if db == 0:
    ...
    ld_calc = (
        (fy / (1.1 * lambda_c * math.sqrt(fc)))
        * ((psi_t * psi_e * psi_s * psi_g) / conf_term)
        * db
    )
    ld = max(ld_calc, 300)
    ...
    ldh_calc = max(
        ((fy * psi_e * psi_t * psi_g) / (23 * lambda_c * math.sqrt(fc))) * db ** 1.5,
        8 * db,
        150,
    )
    return {
...
def calculate_skin_reinforcement(h, d, skin_bar_dia, skin_bar_qty, skin_layers) -> dict:
    """ACI 318-19 side-face (skin) reinforcement check."""
    required = h > 900
    ...
    if provided_layers > 1:
        available_height = zone_height - 2 * 50
...
# ---------------------------------------------------------------------------
# PDF helpers  (FIX #13: module-level utilities, not re-created each call)
# ---------------------------------------------------------------------------

def _pdf_to_float(value):
    try:
        return float(value)
...
def _pdf_status(dc_value) -> str:
    dc_numeric = _pdf_to_float(dc_value)
    if dc_numeric is None:
        return "N/A"
    return "OK" if dc_numeric <= 1.0 else "NG"


def _pdf_wrap_text(pdf, text, width, font_size=9) -> list:
...
def status_card(kind, text, extra_class=""):
    ...
    st.markdown(
        f"<div class='status-card status-{kind}{cls}'>{text}</div>",
        unsafe_allow_html=True,
    )
...
def draw_beam_section(b, h, cover, tie_dia, top_rg, bot_rg, flex, shear, zone, skin=None, skin_bar_dia=10):
    ...
    ax.add_patch(
        patches.Rectangle(
            ...
            b - 2 * (cover + tie_dia / 2),
            h - 2 * (cover + tie_dia / 2),
            fill=False, edgecolor="#c084fc", lw=1.1,
        )
    )
    ax.add_patch(
        patches.Rectangle(
            (cover, cover), b - 2 * cover, h - 2 * cover,
            fill=False, edgecolor="#7a84a0", lw=0.8,
        )
    )
    ...
    def plot_layers(group, from_top=True):
        for n, dia, y_from_face in group.layers:
            ...
            for x in xs:
                ax.add_patch(
                    patches.Circle((x, y), max(dia / 2, 4), facecolor="#fbbf24", edgecolor="#92400e", lw=0.5)
                )
    ...
    for cx, cy in [
        ...
        ax.add_patch(patches.Circle((cx, cy), 5, facecolor="#c084fc", edgecolor="#5b21b6", lw=0.5))
        ...
        for sy in y_vals:
            for offset in x_offsets:
                for base, sign in [(left_base, 1), (right_base, -1)]:
                    ...
                    ax.add_patch(
                        patches.Circle(
                            (sx, sy), max(skin_bar_dia / 2, 3.5),
                            facecolor="#38bdf8", edgecolor="#164e63", lw=0.45,
                        )
                    )

    ax.annotate(
        "", xy=(0, h + pad * 0.25), xytext=(b, h + pad * 0.25),
        arrowprops=dict(arrowstyle="<->", color="#7a84a0", lw=0.55),
    )
    ax.text(b / 2, h + pad * 0.34, f"b={b:.0f}", color="#e8eaf0", ha="center", fontsize=4.6)
    ax.annotate(
        "", xy=(b + pad * 0.55, 0), xytext=(b + pad * 0.55, h),
        arrowprops=dict(arrowstyle="<->", color="#7a84a0", lw=0.55),
    )
...
def draw_force_diagrams(forces, beam_length, df=None, selected_frame="Manual"):
    ...
    if df is not None and not df.empty and {"Station", "M3", "V2"}.issubset(df.columns):
        ...
        ax_m.annotate(
            f"i -{abs(forces['Left']['M']):.0f}", (x[0], m[0]),
            color="#f87171", fontsize=4.8, xytext=(2, -7), textcoords="offset points",
        )
        ax_m.annotate(
            f"mid +{abs(forces['Mid']['M']):.0f}", (x[1], m[1]),
            color="#4f8ef7", fontsize=4.8, xytext=(2, 4), textcoords="offset points",
        )
        ax_m.annotate(
            f"j -{abs(forces['Right']['M']):.0f}", (x[2], m[2]),
            color="#f87171", fontsize=4.8, xytext=(-23, -7), textcoords="offset points",
        )
    ...
    for ax in (ax_m, ax_v):
        ax.legend(
            facecolor="#181c24", edgecolor="#2a3044", labelcolor="#e8eaf0",
            fontsize=3.5, loc="upper left", borderpad=0.2, handlelength=1.6,
        )
...
def create_pdf_report(b, h, fc, fy, fyt, frame_name, zone_data, input_mode) -> bytes:

    def _table_row(col1, col2, col3, col4):
        col_widths = [28, 52, 72, 28]
        values = [str(col1), str(col2), str(col3), str(col4)]
        wrapped_cells = [_pdf_wrap_text(pdf, v, w) for v, w in zip(values, col_widths)]
    ...
    for zone in ["Left", "Mid", "Right"]:
        ...
        if not data:
            ...
            pdf.cell(
                0, 8,
                "DESIGN ABORTED: Reinforcement layers do not fit within the specified section width.",
...
<div class="app-hero">
  ...
  <div class="app-hero-sub">Dark workspace with check cards, torsion indicators, and beam cross-section sketch.</div>
</div>
    """,
    unsafe_allow_html=True,
)
...
input_mode = st.radio(
    "Force input source", ["Manual Input", "SAP2000 CSV Upload"],
...
if use_sap:
    ...
    if df_raw is not None:
        ...
        if "Frame" not in df_raw.columns:
            ...
            grouping_mode = st.radio(
                ...
                horizontal=True, key="grouping_mode",
            ...
            if grouping_mode == "Single frame":
                default_idx = (
                    available_frames.index(st.session_state["selected_frame_single"])
                    if st.session_state["selected_frame_single"] in available_frames
                    else 0
                )
                selected_frame = st.selectbox(
                    "Select beam frame", available_frames,
                    index=default_idx, key="selected_frame_single",
                )
                selected_frames = [selected_frame]
                selected_frame_label = selected_frame
            else:
                default_group = [
                    f for f in st.session_state["selected_frames_group"]
                    if f in available_frames
                ]
                selected_frames = st.multiselect(
                    "Select frames to group", available_frames,
                    default=default_group if default_group else available_frames[: min(2, len(available_frames))],
                    key="selected_frames_group",
                    help="Pick beams with similar section/detailing intent. App will design for the worst force effects.",
                )
                if selected_frames:
            ...
            df = (
                df_raw[df_raw["Frame"].astype(str).isin(selected_frames)].copy()
            ...
            if not df.empty:
                ...
                zone_frames = {
                    "Left":  df_left,
                    "Mid":   df_mid if not df_mid.empty else df,
                    "Right": df_right,
                }
    ...
    st.info(
        f"ACI deflection h_min note: about {beam_length * 1000 / 18.5:.1f} mm for one-end-continuous. "
        "Adjust for actual end conditions."
    )
...
with col_prop:
    ...
    lambda_c     = st.selectbox(
        "Concrete type lambda", [1.0, 0.85, 0.75], key="lambda_c",
        format_func=lambda x: {
            1.0: "1.0 Normal weight",
    ...
    skin_layers   = skin_c3.number_input(
        "Skin layers", min_value=1, step=1, key="skin_layers",
...
with col_rebar:
    ...
    # FIX #11: Build rebar_data and bar_selections together in a single dict;
    # both are populated before the design loop so there is no stale-variable risk.
    rebar_data = {}
    bar_selections = {}
    ...
    for i, zone in enumerate(["Left", "Mid", "Right"]):
        with tabs[i]:
            ...
            # First non-zero layer diameter for development-length calculations
...
if st.session_state.get("design_results_visible", False):
    ...
    st.markdown(
        f"""
<style>
.three-zone-scale.zone-title      {{ font-size: calc(18px * {RESULTS_FONT_SCALE}); }}
.three-zone-scale.status-card     {{ font-size: calc(9px  * {RESULTS_FONT_SCALE}); }}
.three-zone-scale.mini-metric-label {{ font-size: calc(5.5px * {RESULTS_FONT_SCALE}); }}
.three-zone-scale.mini-metric-value {{ font-size: calc(12px  * {RESULTS_FONT_SCALE}); }}
.three-zone-scale.mini-metric-delta {{ font-size: calc(5.5px * {RESULTS_FONT_SCALE}); }}
.three-zone-scale.check-row       {{ font-size: calc(14px * {RESULTS_FONT_SCALE}); }}
.three-zone-scale.check-detail    {{ font-size: calc(12px * {RESULTS_FONT_SCALE}); }}
.three-zone-scale.badge           {{ font-size: calc(11px * {RESULTS_FONT_SCALE}); }}
</style>
        """,
        unsafe_allow_html=True,
    )
    st.markdown(
        "<div class='section-band'>Three-Zone Cross Sections and Calculations</div>",
        unsafe_allow_html=True,
    )
    ...
    for idx, zone in enumerate(["Left", "Mid", "Right"]):
        with result_columns[idx]:
            ...
            if top_rg.width_req > b or bot_rg.width_req > b:
                status_card(
                    "fail",
                    f"Bars do not fit in {b} mm width. "
                    f"Top requires {top_rg.width_req:.1f} mm, bottom requires {bot_rg.width_req:.1f} mm.",
            ...
            # ------------------------------------------------------------------
            # FIX #7: dt must be the FARTHEST tension layer from the compression face.
            #
            # get_rebar_group always measures distances from its own NEAR face:
            #   top group  → distances from TOP face
            #   bot group  → distances from BOTTOM face
            #
            # extreme_fiber = distance from near face to the FIRST (outermost) layer.
            # centroid      = distance from near face to the area-weighted centroid.
            #
            # At supports (negative moment): top bars are in TENSION.
            #   d  = h − top_rg.centroid       (compression face = bottom)
            #   dt = h − top_rg.extreme_fiber  (furthest tension layer = first top layer
            #                                   which is the one CLOSEST to the top face,
            #                                   i.e. furthest from the compression face)
            #   d' = bot_rg.centroid            (compression steel from bottom face)
            #
            # At midspan (positive moment): bottom bars are in TENSION.
            #   d  = h − bot_rg.centroid
            #   dt = h − bot_rg.extreme_fiber  (same logic: first bottom layer is the one
            #                                   closest to the bottom face = furthest from
            #                                   the compression face at the top)
            #   d' = top_rg.centroid
            # ------------------------------------------------------------------
            if zone == "Mid":
                As_tens  = bot_rg.area
                As_comp  = top_rg.area
                d        = h - bot_rg.centroid
                dt       = h - bot_rg.extreme_fiber   # FIX #7
                d_prime  = top_rg.centroid
            else:
                As_tens  = top_rg.area
                As_comp  = bot_rg.area
                d        = h - top_rg.centroid
                dt       = h - top_rg.extreme_fiber   # FIX #7
            ...
            res_shear = calculate_shear_torsion(
                ...
                Vu_design, Tu_design, n_legs, bar_v, lambda_c,
            )
            ...
            with m3:
                mini_metric(
                    "Stirrups", f"{n_legs}-{bar_v_name}",
                    f"@ {res_shear['final_s']} mm" if res_shear["final_s"] else "FAIL",
                    extra_class="three-zone-scale",
                )
            with m4:
                mini_metric("Strain", f"{res_flex['eps_t']}", res_flex["strain_class"], extra_class="three-zone-scale")

            fig = draw_beam_section(
                b, h, cover_clear, bar_v,
                top_rg, bot_rg, res_flex, res_shear, zone, skin, skin_bar_dia,
            )
            ...
            check_row("Flexure phiMn >= Mu",         res_flex["phi_Mn"] >= Mu,
                      f"{m_combo}; {res_flex['phi_Mn']} >= {Mu:.1f} kNm",                          extra_class="three-zone-scale")
            check_row("Minimum As",                  res_flex["passes_As_min"],
                      f"As = {As_tens:.1f}; As,min = {res_flex['As_min']} mm2",                    extra_class="three-zone-scale")
            check_row("Tension-controlled",          res_flex["is_ductile"],
                      f"eps_t = {res_flex['eps_t']}; phi = {res_flex['phi']}",
                      warn=not res_flex["is_ductile"] and res_flex["phi_Mn"] >= Mu,                 extra_class="three-zone-scale")
            check_row("Max As (tension-controlled)", res_flex["passes_As_max_tc"],
                      f"As = {As_tens:.1f}; As,max,tc = {res_flex['As_max_tc']} mm2",              extra_class="three-zone-scale")
            check_row("Shear phiVn >= Vu",           res_shear["phi_Vn"] >= Vu_design,
                      f"{v_combo}; {res_shear['phi_Vn']} >= {Vu_design:.1f} kN",                   extra_class="three-zone-scale")
            check_row("Transverse spacing",          res_shear["final_s"] > 0,
                      f"s exact = {res_shear['s_exact']} mm; s max = {res_shear['s_max']} mm",     extra_class="three-zone-scale")
            check_row("Torsion threshold",           not res_shear["needs_torsion"],
                      f"{t_combo}; Tu = {Tu_design:.1f} kNm; phiTth = {res_shear['T_th']} kNm",
                      warn=res_shear["needs_torsion"],                                               extra_class="three-zone-scale")

            skin_detail = (
                f"{skin['layers']} layer(s) of {skin['bars_per_layer']}-{skin_bar_name}; "
                f"h = {h:.0f} mm <= 900 mm, not required"
                if not skin["required"]
                else
                f"{skin['layers']} layer(s) of {skin['bars_per_layer']}-{skin_bar_name}; "
                f"s = {skin['spacing']} <= {skin['s_limit']} mm"
            )
            check_row("ACI side-face skin bars", skin["spacing_ok"], skin_detail, extra_class="three-zone-scale")

            # Face-by-face torsion longitudinal steel check (MacGregor / ACI 318)
            if res_shear["needs_torsion"]:
                ...
                if torsion_long_ok:
                    torsion_long_detail = (
                        f"Tension face & sides OK. "
                        f"(Al req side: {Al_req_sides:.0f} mm2, face: {Al_req_face:.0f} mm2)"
                    )
                else:
                    torsion_long_detail = (
                        f"FAIL: Tension face prov {tension_face_prov:.0f}/{tension_face_req:.0f} mm2. "
                        f"Sides prov {side_face_prov:.0f}/{Al_req_sides:.0f} mm2"
                    )
            else:
            ...
            check_row(
                ...
                torsion_long_ok, torsion_long_detail,
                extra_class="three-zone-scale",
            )

            # Optional calculation summary table
                ...
                st.dataframe(
                    pd.DataFrame(
                        [
                            ...
                            ("dt",                     f"{dt:.1f} mm",                       "Extreme tension fibre depth"),
                            ...
                            ("lambda_s (ref only)",    res_shear["lambda_s"],                "ACI 318-19 size-effect factor (Av < Av,min case)"),
                            ...
                            ("Skin bars",              skin_detail,                           "ACI 318 side-face long. reinf. for h > 900 mm"),
                            ("Skin Al for torsion",    f"{skin['area_total']} mm2",           "Counted as long. torsion steel if enclosed by closed stirrups"),
            ...
            stirrup_text = (
                f"{n_legs}-DB{bar_v} @ {res_shear['final_s']} mm (D/C: {dc_shear})"
                if res_shear["final_s"] > 0 else "FAILS"
            )
            pdf_zone_data[zone] = {
                "Mu": round(Mu, 1), "M_combo": m_combo,
                "phi_Mn": res_flex["phi_Mn"], "DC_flex": dc_flex,
                "Vu": round(Vu_design, 1), "V_combo": v_combo,
                "Tu": round(Tu_design, 1), "T_combo": t_combo,
                "DC_shear": dc_shear, "phi_Vn": res_shear["phi_Vn"],
                "stirrups": stirrup_text,
                "torsion_status": "Required" if res_shear["needs_torsion"] else "Below threshold",
                "Al_req": res_shear["Al_req"],
                "skin_Al": skin["area_total"], "skin_detail": skin_detail,
                "eps_t": res_flex["eps_t"], "phi": res_flex["phi"],
                "strain_class": res_flex["strain_class"],
                "dev_top": dev_top["ldh"], "dev_top_lap": dev_top["lap"],
                "dev_bot": dev_bot["lap"],
            }
            summary_rows.append(
                {
                    "Zone": zone,
                    "Mu (kNm)": round(Mu, 1), "M Combo": m_combo,
                    "phiMn (kNm)": res_flex["phi_Mn"], "Flexure D/C": dc_flex,
                    "Vu (kN)": round(Vu_design, 1), "V Combo": v_combo,
    ...
    if summary_rows:
        ...
        st.download_button(
            ...
            data=pdf_bytes,
            file_name=f"Beam_{safe_name}_Report.pdf",
...
st.markdown(
    """
<div class="notice">
  <strong>Scope note:</strong> Torsion web-crushing interaction, hook detailing, seismic detailing,
  development/anchorage conditions, and final bar layout should be verified against the official ACI
  standard before issuing drawings.
</div>
    """,
    unsafe_allow_html=True,
)
