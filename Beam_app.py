...
with st.expander("Save / Load Full Workspace"):
    workspace_bytes = build_workspace_db_bytes()
    st.download_button(
        "Save full workspace (.db)",
        data=workspace_bytes,
        file_name="rc_beam_workspace.db",
        mime="application/octet-stream",
        help="Includes all keyed inputs plus SAP frame-force rows (if available).",
    )
    uploaded_workspace = st.file_uploader("Load full workspace (.db)", type=["db", "sqlite", "sqlite3"], key="workspace_loader")
    if uploaded_workspace is not None:
        try:
            load_workspace_db(uploaded_workspace)
            st.success("Workspace loaded. Inputs and SAP data restored.")
            st.rerun()
        except Exception:
            st.error("Could not read workspace database. Please upload a valid .db file generated by this app.")
